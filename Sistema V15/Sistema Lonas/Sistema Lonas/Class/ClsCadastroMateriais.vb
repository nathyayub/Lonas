Public Class ClsCadastroMateriais
    ...
    Public Sub Gravar(novo As Boolean)
        If novo = True Then
            ...
            sql = "select max(codigo) as codigo from tabitensfornecido"
            m_codi = objbanco.buscar_ultimoRegistro(sql)
        Else
            sql = "Update tabitensfornecido set produto='" & m_nome & "', unimed='" & m_uni & "', despro='" & m_desc & "', qtdmin='" & m_min & "', qtdmax='" & m_max & "' where codigo=" & m_codi
            objbanco.executar_comando(sql)
        End If
    End Sub
    ...
    Public Function Excluir(chave As Integer) As Boolean
        If MessageBox.Show("Deseja excluir?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            sql = "Delete from tabitensfornecido where codigo=" & chave
            objbanco.executar_comando(sql)
            Return True
        End If
    End Function
    ...
    Public Function LocalizarPorCodigoOuNome(campo As String) As Boolean
        If IsNumeric(campo) Then
            sql = "select * from tabitensfornecido where codigo=" & campo
        Else
    ...
    Public Sub atualizarEstoque(codpro As Integer, qtd As Integer)
        sql = "update tabitensfornecido set qtdatu=qtdatu+" & qtd & " where codigo=" & codpro
        objbanco.executar_comando(sql)
    End Sub
